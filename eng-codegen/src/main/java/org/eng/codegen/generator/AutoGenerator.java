 ...
 * the License.
 */
package org.eng.codegen.generator;

import org.eng.codegen.generator.config.ConstVal;
import org.eng.codegen.generator.config.FileOutConfig;
import org.eng.codegen.generator.config.FreemarkerData;
import org.eng.codegen.generator.config.TemplateConfig;
import org.eng.codegen.generator.config.builder.ConfigBuilder;
...
public class AutoGenerator extends AbstractGenerator {
	...
	public void execute() {
		...
		// 获取上下文
		Map<String, FreemarkerData> ctxData = analyzeData(config);
		// 循环生成文件
		for (Map.Entry<String, FreemarkerData> ctx : ctxData.entrySet()) {
	 ...
	 * @return 解析数据结果集
	 */
	private Map<String, FreemarkerData> analyzeData(ConfigBuilder config) {
		List<TableInfo> tableList = config.getTableInfoList();
		Map<String, String> packageInfo = config.getPackageInfo();
		Map<String, FreemarkerData> ctxData = new HashMap<String, FreemarkerData>();
	 ...
	 *            vm上下文
	 */
	private void batchOutput(String entityName, FreemarkerData context) {
		try {
	 ...
	 *            文件生成的目录
	 */
	private void vmToFile(FreemarkerData context, String templateName, String outputFile) throws IOException {
		...
		FileOutputStream controllerOutput = new FileOutputStream(outputFile);
		/* 合并数据模型和模版 */
		Writer controllerWriter = new OutputStreamWriter(controllerOutput);
		try {
			template.process(context.getData(), controllerWriter);
		...
		logger.debug("模板:" + templateName + ";  文件:" + outputFile);
	}
	...
	private String getTemplatePath(){
		String path = getGlobalConfig().getTemplatesPath();
		if(StringUtils.isEmpty(path)){
			path = this.getClass().getClassLoader().getResource("").getPath()+File.separator+ConstVal.DEFAULT_TEMPLATE_PATH;
		}
		return path;
	}

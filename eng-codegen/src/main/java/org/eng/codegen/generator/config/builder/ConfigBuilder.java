 ...
 * the License.
 */
package org.eng.codegen.generator.config.builder;
...
import org.eng.codegen.generator.config.po.TableField;
...
import org.eng.codegen.generator.config.rules.QuerySQL;
...
public class ConfigBuilder {
	...
	public ConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
		...
		if (null == packageConfig) {
			handlerPackage(this.template, new PackageConfig());
		} else {
	...
	private void handlerPackage(TemplateConfig template, PackageConfig config) {
		...
		if (StringUtils.isNotEmpty(template.getEntity())) {
			pathInfo.put(ConstVal.ENTITY_PATH, joinPath(getGlobalConfig().getEntityOutputDir(), packageInfo.get(ConstVal.ENTITY)));
		}
		...
		if (StringUtils.isNotEmpty(template.getServiceImpl())) {
			pathInfo.put(ConstVal.SERVICEIMPL_PATH, joinPath(getGlobalConfig().getServiceOutputDir(), packageInfo.get(ConstVal.SERVICEIMPL)));
		}
		if (StringUtils.isNotEmpty(template.getController())) {
			pathInfo.put(ConstVal.CONTROLLER_PATH, joinPath(getGlobalConfig().getControllerOutputDir(), packageInfo.get(ConstVal.CONTROLLER)));
		}
	}
				...
				if (StringUtils.isNotEmpty(tableName)) {
					...
					if (isInclude) {
						for (String includeTab : config.getInclude()) {
							//修改为前缀匹配
							if (tableName.startsWith(includeTab)) {
	...
	private List<TableField> getListFields(String tableName, NamingStrategy strategy) throws SQLException {
		...
		while (results.next()) {
			...
			setExtraAttr(field.getComment(), field);
			fieldList.add(field);
		}
		return fieldList;
	}
	private void setExtraAttr(String content, TableField field){
		...
		for(String temp : attrs){
			if(temp.contains(":")){
				attrMap.put(temp.split(":")[0], temp.split(":")[1]);
			}
		}
		field.setLabel(attrMap.get("label"));
		field.setInputType(attrMap.get("type"));
		field.setValid(attrMap.get("valid"));
	}
